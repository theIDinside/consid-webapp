// Settings for the form, depending on the selected library item type
const EDIT_FORM_TYPE = {
    PLAYABLE: { enable: { field: "RunTimeMinutes", group: "runTimeMinutesInput" }, disable: { field: "Pages", group: "pagesInput" } },
    BOOK: { enable: { field: "Pages", group: "pagesInput" }, disable: { field: "RunTimeMinutes", group: "runTimeMinutesInput" } },
}
...
async function populateCategoriesList(elementId = "categoryNameSelect") {
    ...
    if (e) {
        return fetch("/Category/GetCategories").then(res => res.json()).then(result => {
